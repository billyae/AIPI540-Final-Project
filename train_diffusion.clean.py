...
class ImageLabelDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return {
            ...
            "controlnet_cond": self.transform(lbl),
...
def load_controlnet_pipeline(config: TrainingConfig):
    ...
    controlnet = ControlNetModel.from_pretrained(
        ...
        config.controlnet_ckpt, torch_dtype=torch.float16
...
def train_one_epoch(pipe, loader, optimizer, scheduler, accelerator, config: TrainingConfig):
    ...
    for batch in loader:
        ...
        pixel_vals = batch["pixel_values"].to(config.device)

        cond_imgs  = batch["controlnet_cond"].to(config.device)
